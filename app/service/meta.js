...
class MetaService extends Service {
  ...
  async findCategoryById(cid) {
    const sql = 'select m.mid, m.name, m.slug from meta as m join relationship as r where r.cid = ? and r.mid = m.mid and m.type = "category"';
  ...
  async findTagsById(cid) {
    const sql = 'select m.mid, m.name, m.slug from meta as m join relationship as r where r.cid = ? and r.mid = m.mid and m.type = "tag"';
    const res = await this.app.mysql.query(sql, [ cid ]);
    return res;
  }
